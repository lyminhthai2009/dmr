...
abstract class DragonMountingComponent extends DragonOwnershipComponent {
    ...
    protected boolean canAddPassenger(Entity passenger) {
        // VIP: Cho phép tối đa 3 người (1 lái, 2 ngồi sau)
        return this.getPassengers().size() < 3;
    }

    /**
     * Positions the rider(s) on the dragon.
     */
    @Override
    public void positionRider(Entity passenger, Entity.MoveFunction callback) {
        if (!this.hasPassenger(passenger)) return;

        // VIP: Tính toán vị trí ngồi dựa trên thứ tự lên rồng
        int index = this.getPassengers().indexOf(passenger);
        
        // Tọa độ gốc tùy chỉnh của VIP
        double zOffset = -0.7 * index; // Người sau ngồi lùi lại 0.7 block
        double yOffset = index * 0.1;  // Người sau ngồi cao hơn chút

        Vec3 vec3 = getPassengerRidingPosition(passenger);
        Vec3 vec31 = passenger.getVehicleAttachmentPoint(this);
        Vec3 basePos = new Vec3(vec3.x - vec31.x, vec3.y - vec31.y, vec3.z - vec31.z);
        
        // Xoay vị trí offset theo hướng nhìn của rồng
        Vec3 rotatedOffset = new Vec3(0, yOffset, zOffset).yRot(-this.getYRot() * ((float)Math.PI / 180F));
                            
        callback.accept(passenger, basePos.x + rotatedOffset.x, basePos.y + rotatedOffset.y, basePos.z + rotatedOffset.z);

        // fix rider rotation
        if (passenger instanceof LivingEntity riddenByEntity) {
    ...
    public LivingEntity getControllingPassenger() {
        ...
        // VIP: Chỉ có chủ nhân ngồi ở ghế số 0 mới có quyền điều khiển bay/di chuyển rồng
        return (first instanceof LivingEntity driver && isOwnedBy(driver)) ? driver : null;
    }
}